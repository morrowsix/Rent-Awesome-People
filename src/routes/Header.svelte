...
>
  ...
  <!-- Mobile Nav -->
  <div
    class="flex-grow items-center text-center pl-5 menu:hidden transition-all overflow-hidden {mobileNavVisible} pt-5"
  >
    <ul class="flex flex-grow gap-4 flex-col list-style-none font-medium">
      {#each navlinks as navlink}
        <li
          class={path === navlink.url ? "text-secondary" : ""}
          on:click={toggleMobileMenu}
        >
          <a class=" hover:text-secondary" href={navlink.url}>{navlink.title}</a
          >
        </li>
      {/each}
    </ul>
